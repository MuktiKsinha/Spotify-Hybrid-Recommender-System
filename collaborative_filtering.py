...
def filter_songs_data(songs_data: pd.DataFrame, track_ids: list, save_df_path: str) ->pd.DataFrame:
    """
    Filter the songs data for the given track track_ids
    """
    # filter data based on track ids
...
def create_interation_matrix(history_data:dd.DataFrame,track_ids_save_path,save_matrix_path) ->csr_matrix:
    # make a copy
    df = history_data.copy()

    #convert playcount colomn into str
    df['playcount'] = df['playcount'].astype(np.float64)

    #convert string coloumn into categorical coloumn
    ...
    #add the index coloumn to data frame
    df = df.assign(
...
def collaborative_recommendation(song_name,artist_name,track_ids,songs_data,interaction_matrix,k=5):
    ...
    #lower the artist name
    artist_name =artist_name.lower()

    #fetch the row from the song data
    song_row = songs_data.loc[(songs_data["name"] == song_name) & (songs_data["artist"] == artist_name)]

    # track_id of input song
    input_track_id = song_row['track_id'].values.item()

    # fetch the index position of track id
...
def main():
    ...
    # create the interaction matrix
    create_interation_matrix(user_data,track_ids_save_path,interaction_matrix_save_path)
